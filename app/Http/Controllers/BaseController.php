...
class BaseController extends Controller
{
    ...
    if ($validator->fails()) {
        ...
        // Check if reg_id matches membership_no
       
        $regId = $request->reg_id;
        
        // Split prefix and number
        if (strpos($regId, 'RHIN') !== 0) {
            return response()->json([
                'errors' => ['reg_id' => ['Invalid Registration ID format']]
            ], 422);
        }
        
        $number = substr($regId, 4); // extract last 4 digits
        $number = ltrim($number, '0'); // remove leading zeros
        
        // Try matching membership_no first
        $member = Member::where('membership_no', $number)->first();
        
        if (!$member) {
            // If not found, match by ID
            $member = Member::find($number);
        }
        
        if (!$member) {
            return response()->json([
                'errors' => ['reg_id' => ['Invalid Registration ID']]
            ], 422);
        }
  ...
  //  print_r($category_ids);
            ...
            TAbstract::where('id', $abstract['id'])->update(['mail_send'=>2]);
             return response()->json(['status'=>true, 'message'=>'Abstarct Mail has been sent successfully']);
    ...
    {
        //dd($search);
        $member = Member::orwhere('email', 'LIKE', "%$search%")->orwhere('mobile', 'LIKE', "%$search%")->select('id', 'membership_no', 'category', 'profile', 'name', 'email', 'mobile', 'code', 'qr_code')->first(); 
        $dinner = '';
        $room_tarrif = '';
        $training = '';
          $gala_dinner = Dinner::where('status', 1)->orderBy('id', 'asc')->first();
        ...
        //dd($room);
        return response()->json(['member'=>$member, 'dinner'=>isset($dinner) && !empty($dinner) ? $dinner : 0,'gala_dinner' => $gala_dinner,'room_tarrif' => $room_tarrif, 'room'=>isset($room) && !empty($room) ? $room : 0]);
    }
    public function registrationStore(Request $request)
    {
        ...
        //   'name'       => 'required_without:first_name|string|max:100',
            'mobile'=>'required', // not all zeros
         ...
         //   'name.required_without'=>'Name is required if First Name is empty',
            ...
            //   'mobile.regex' => 'Mobile Number cannot be all zeros',
            'email.required'=>'Email is required',
        ...
        Log::channel('webhook')->info("Order test");
     //   try {
            // if(isset($request->id) && !empty($request->id)){
            ...
            // ]));
                if ($request->hasFile('profile')) {
                        ...
                        $amount = $request->total;$type = "non_vip";
                        $notes = array_merge(
                        $request->only([
                            'id',
                            'name','first_name','middle_name','last_name','country_code',
                            'email',
                            'mobile',
                            'category',
                            'membership_no',
                   ...
                   $api = new Api(config('services.razorpay.key'), config('services.razorpay.secret'));
                    ...
                    $order = $api->order->create([
                     ...
                     Log::channel('webhook')->info("{$request->category} Order Create: " . json_encode($order->toArray()));
                     ...
                     Order::create([
                            ...
                            //  return response()->json([
                            //     'status'   => 'success',
                            //     'redirect' => route('razorpay.checkout', [
                            //         'order_id' => $order['id'],
                            //         'amount' =>  (int) round($amount * 100),
                            //         'currency' => $request->currency,
                            //         'key' => config('services.razorpay.key'),'userName' => $validatedData['name'] ?? '',
                            //         'userEmail' => $validatedData['email'] ?? '','userMobile' => $validatedData['mobile'] ?? '', 'allData' => $request->all(),'receipt'    => $receiptData
                            //     ])
                            // ]);
                                $mobile_hidden = $request->mobile_hidden;
                             return view('razorpay_checkout', [
                                'order_id' => $order['id'],
                                'amount' =>  (int) round($amount * 100),
                                'currency' => $request->currency,
                                'key' => config('services.razorpay.key'),'userName' => $validatedData['name'] ?? '',
                                'userEmail' => $validatedData['email'] ?? '','userMobile' => $mobile_hidden ?? '', 'allData' => $request->all(),'receipt'    => $receiptData
                            ]);
                } catch (\Exception $e) {
                     ...
                     Cache::put('user_receipt_' . $receiptData, $notes, now()->addMinutes(30));
                       Order::create([
                        ...
                        return view('razorpay_checkout', [
                            ...
                            'userMobile' => $validatedData['mobile'] ?? '',
                            'allData'    => $request->all(),
                            'receipt'    => $receiptData
                        ]);
                        //    return back()->with(['status'=>false, 'msg'=>'Failed to Stored']);
...
{
    $order = Order::where('razorpay_order_id', $request->order_id)->firstOrFail();
    ...
    public function packageStore(Request $request)
    {
        ...
        try {
            ...
            if(isset($request->gala_dinner) && !empty($request->gala_dinner)){
             Log::channel('webhook')->info("Gala Dinner Request Data", $request->all());
            }
            if(isset($request->room_tarrif_txt) && !empty($request->room_tarrif_txt)){
                Log::channel('webhook')->info("Room Tarrif Request Data", $request->all());
            }
            $user_type = "update_member";
            $notes = array_merge(
                        $request->only([
                            'member_id','gala_dinner','gala_dinner_price','room_tarrif_txt','room_tarrif_price','room_tarrif_date','person','total'
                        ]),['user_type' => $user_type]
                    );
                 $api = new Api(config('services.razorpay.key'), config('services.razorpay.secret'));
                 $receiptData = 'RCPT_' . time();
                 ...
                 $order = $api->order->create([
                    ...
                    Log::channel('webhook')->info("Order Create: " . json_encode($order->toArray()));
                    Cache::put('user_order_' . $order['id'], $notes, now()->addMinutes(30));
                     Order::create([
                                'razorpay_order_id' => $order['id'],
                                'receipt' => $receiptData,
    ...
    public function traningStore(Request $request)
    {
        ...
        try {
            ...
            if(isset($request->training) && !empty($request->training)){
             Log::channel('webhook')->info("Training Request Data", $request->all());
            }
            $user_type = "update_member";
            $notes = array_merge(
                        $request->only([
                            'member_id','training','total'
                        ]),['user_type' => $user_type]
                    );
                 $api = new Api(config('services.razorpay.key'), config('services.razorpay.secret'));
                 $receiptData = 'RCPT_' . time();
    ...
    public function generatePDF($id)
    {
        ...
        $package_count = AdditionalPackage::where('member_id', $id)->where('payment_status', 'paid')->select('gala_dinner_price', 'room_tarrif_price')->count();
    ...
    public function vipStore(Request $request)
    {
        ...
        try {
            ...
            $profile = '';
            $payment_screen_shot = '';$receiptData = 'RCPT_' . time();
            if (request()->hasFile('profile')) {
                $profile = $this->imageUpload(request()->file('profile'), '/upload/member/');
            }
            if (request()->hasFile('payment_screen_shot')) {
            ...
            if (!empty($request->total) && (float)$request->total != 0.00) {
                Log::channel('webhook')->info("VIP Request Data", $request->all());
                ...
                $notes = array_merge(
                        $request->only([
                            'id',
                            'name','first_name','middle_name','last_name','type','country_code',
                            'email',
                            'mobile',
                            'category',
                            'membership_no',
                        ...
                        [
                            'profile_path' => $profile ?? null,'payment_screen_shot_path' => $payment_screen_shot ?? null,
                        ],
                         ['receipt' => $receiptData]
                    );
                try{
                 ...
                 Log::channel('webhook')->info("{$request->category} VIP Order Create: " . json_encode($order->toArray()));
                 Cache::put('user_order_' . $order['id'], $notes, now()->addMinutes(30));
                     Order::create([
                                'razorpay_order_id' => $order['id'],
                                'receipt' => $receiptData,
