...
class SpouseExport implements FromView, WithColumnWidths
{
    /**
    ...
    */
    ...
    public function view(): View
    {
        if(isset($this->from_date) && !empty($this->from_date) && isset($this->to_date) && !empty($this->to_date) && isset($this->option) && !empty($this->option)) {
            ...
            /*
            $members = Member::whereNull('type')->where('category', 'spouse-accompanying')->where('status', 1);
            ...
            //start member id
            $member_id = Member::whereNull('type')->where('category', 'spouse-accompanying')->where('status', 1);
            $from = $this->from_date;
            $to = $this->to_date;
            if(isset($from) && !empty($from) && isset($to) && !empty($to)){
                $from = date("Y-m-d h:i:s", strtotime($this->from_date));
            ...
            if(isset($this->option) && !empty($this->option)){
             ...
             if(isset($this->option) && !empty($this->option)){
                if($this->option == 'all'){
                    ...
                    $members =  Member::where('category', 'spouse-accompanying')->select('id', 'category', 'name', 'first_name', 'middle_name', 'last_name', 'mobile', 'become_aris_member', 'become_aris_member_price', 'plan_id', 'plan_date', 'plan_amount', 'email', 'medical_reg_no', 'designation', 'institute', 'code', 'address', 'city', 'state', 'pincode', 'country', 'created_at')->where('status', 1)->get();
                    ...
                    $members =  Member::where('category', 'spouse-accompanying')->select('id', 'category', 'name', 'first_name', 'middle_name', 'last_name', 'mobile', 'become_aris_member', 'become_aris_member_price', 'plan_id', 'plan_date', 'plan_amount', 'email', 'medical_reg_no', 'designation', 'institute', 'code', 'address', 'city', 'state', 'pincode', 'country', 'created_at')->where('become_aris_member_price', '!=', '0.00')->where('status', 1)->get();
                }
            }           
        }
        return view('admin.spouse.export', ['members' => $members, 'from_date'=>$this->from_date, 'to_date'=>$this->to_date, 'option'=>$this->option]);
    }
