...
@section('title_bar', 'International Members')
@section('breadcrumb', 'International Members')
@section('maincontent')
<style>
    a{
                ...
                <div class="col-lg-6">
                    <div class="row mt-4">
                        ...
                        <div class="col-lg-1" style="padding:0px 15px 0px 15px;">
                            <a class="btn btn-dark" href="{{ url('admin/international') }}"><i class="fa fa-refresh"></i></a>
                        </div>
                ...
                <div class="col-xl-12">
                    <div class="card" id="accordion-one">
                        <div class="card-header flex-wrap px-3">
                            <div>
                                <h4 class="card-title">International Member</h4>
                            </div>
                        </div>
                        <!--tab-content-->
                            ...
                            <div class="table-responsive">
                                <table id="example1" class="table w-100">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td><a href="{{ url('admin/international/detail') }}{{ '/'.$m->id }}"><span class="text-primary font-w600">{{ isset($m->membership_no) && !empty($m->membership_no) ? 'RHIN'.'0000'.$m->id : 'RHIN'.'0000'.$m->id }}</span></a></td>
                                            <td>{{ $m->name }}</td>
                                            <td>{{ isset($m->created_at) && !empty($m->created_at) ? date('M d, Y',strtotime($m->created_at)) : '--' }}</td>
                                            <td>{{ isset($m->code) ? $m->code : '' }} {{  $m->mobile }}</td>
                                            <td>{{ $m->city }}</td>
                                            <td>
                                                <div class="d-flex">
                                                    <a href="{{ url('admin/international/detail') }}{{ '/'.$m->id }}" data-id="{{ $m->id }}" class="btn btn-primary shadow btn-xs sharp me-1"> <i class="fa-regular fa-eye"></i></a>
                                                    <a href="{{ url('admin/international/edit') }}{{ '/'.$m->id }}" class="btn btn-secondary shadow btn-xs sharp me-1"><i class="fa fa-edit"></i></a>
                                                    ...
                                                    <a href="javascript:void(0)" class="btn btn-primary shadow btn-xs sharp reg_confiramtion_mail" data-id="{{ $m->id }}" style="margin-left:5px;"><i class="fa fa-user-plus" aria-hidden="true"></i></a>
                                                </div>
...
table = new DataTable('#example1', {
    ...
    $(document).on("click",".delete",function() {
        ...
        $('#delrecord #id').val(id);
        $('#delrecord #delete-product').attr('action', "{{ url('admin/delete-international') }}");
    });
    $(document).on("click","#search-btn",function() {
        $('#search').attr("action", "{{ url('admin/international') }}");
        $('#search').submit();
    });
    $(document).on("click","#export-btn",function() {
        $('#search').attr("action", "{{ url('admin/international/export') }}");
        $('#search').submit();
    });
