...
@section('breadcrumb', 'VIP Members Detail')
@section('maincontent')
<div class="content-body">
                        ...
                        <div class="d-flex justify-content-between">
                            <div class="user">
                                ...
                                <div>
                                    ...
                                    <h5 class="text-primary font-w600">ID - {{ $member->membership_no}} </h5>
                                </div>
                            </div>
    ...
    <div class="col-xl-12">
        <div class="card">
            ...
            <div class="card-body">
                <form name="profile_update" id="profile_update" method="POST" action="{{ url('admin/vip-members-update') }}" enctype='multipart/form-data'>
                    @csrf
                ...
                <div class="row">
                    <div class="col-xl-3 col-lg-4">
                        ...
                        <div class="mb-3 mt-3">
                            <label for="first_name" class="form-label text-primary">First Name<span
                                class="required">*</span></label>
                            <input type="text" class="form-control" id="first_name" name="first_name" placeholder="" value="{{isset($member->first_name) && !empty($member->first_name) ? $member->first_name : '' }}">
                        </div>
                        <div class="mb-3 mt-3">
                            <label for="middle_name" class="form-label text-primary">Middle Name<span
                                class="required">*</span></label>
                            <input type="text" class="form-control" id="middle_name" name="middle_name" placeholder="" value="{{isset($member->middle_name) && !empty($member->middle_name) ? $member->middle_name : '' }}">
                        </div>
                        <div class="mb-3 mt-3">
                            <label for="last_name" class="form-label text-primary">Last Name<span
                                class="required">*</span></label>
                            <input type="text" class="form-control" id="last_name" name="last_name" placeholder="" value="{{isset($member->last_name) && !empty($member->last_name) ? $member->last_name : '' }}">
                        </div>
                    ...
                    <div class="col-xl-9 col-lg-8">
                        <div class="row">
                            ...
                            @if(isset($additional_package->gala_dinner) && !empty($additional_package->gala_dinner) && isset($additional_package->gala_dinner_price) && !empty($additional_package->gala_dinner_price))
                            <div class="col-xl-6 col-sm-6">
                                <div class="mb-3">
                                    <label for="email" class="form-label text-primary">Gala Dinner<span class="required">*</span></label>
                                    <p>{{isset($additional_package->gala_dinner) && !empty($additional_package->gala_dinner) ? ucfirst($additional_package->gala_dinner) : '' }} </p>
                                </div>
                            </div>
                            <div class="col-xl-6 col-sm-6">
                                <div class="mb-3">
                                    <label for="email" class="form-label text-primary">Date<span class="required">*</span></label>
                                    <p>{{isset($additional_package->dinner_start_date) && !empty($additional_package->dinner_start_date) ? date("d M, Y h:i a", strtotime($additional_package->dinner_start_date)) : '' }} - {{isset($additional_package->dinner_end_date) && !empty($additional_package->dinner_end_date) ? date("d M, Y h:i a", strtotime($additional_package->dinner_end_date)) : '' }}</p>
                                </div>
                            </div>
                            @endif
                            @if(isset($additional_package->room_tarrif_price) && !empty($additional_package->room_tarrif_price) && $additional_package->room_tarrif_price != 0.00)
                            <div class="col-xl-6 col-sm-6">
                                <div class="mb-3">
                                    <label for="email" class="form-label text-primary">Residential Package<span class="required">*</span></label>
                                    <p>{{isset($additional_package->room_tarrif) && !empty($additional_package->room_tarrif) ? ucfirst($additional_package->room_tarrif) : '' }} </p>
                                </div>
                            </div>
                            <div class="col-xl-6 col-sm-6">
                                <div class="mb-3">
                                    <label for="email" class="form-label text-primary">Date<span class="required">*</span></label>
                                    <p>
                                        @php $room_dates = isset($additional_package->room_dates) && !empty($additional_package->room_dates) ? explode(',',$additional_package->room_dates) : '' @endphp
                                        @foreach($room_dates as $r_date)
                                        {{isset($r_date) && !empty($r_date) ? $r_date : '' }} - 
                                        @endforeach
                                        <br/>No of Days : {{isset($additional_package->no_of_days) && !empty($additional_package->no_of_days) ? $additional_package->no_of_days : 0 }}</p>
                                </div>
                            </div>
                            @php $person = isset($additional_package->person) && !empty($additional_package->person) ? json_decode($additional_package->person) : [] @endphp
                            ...
                            <div class="col-xl-6 col-sm-6">
                                <div class="mb-3">
                                    ...
                                    <p>{{isset($member->name) && !empty($member->name) ? $member->name : '' }}<br/>{{isset($member->mobile) && !empty($member->mobile) ? $member->code.' '.$member->mobile : '' }}</p>
                                </div>
                            </div>
                            <div class="col-xl-6 col-sm-6">
                                <div class="mb-3">
                            ...
                            <div class="col-xl-6 col-sm-6">
                                ...
                                <div class="mb-3">
                                    <label for="diet" class="form-label text-primary">Cusine <span class="required">*</span></label>
                                    <select class="form-control" id="diet" name="diet">
                                        <option value="Tamilnadu Cusine" @if(isset($member->diet) && !empty($member->diet) && $member->diet == 'Tamilnadu Cusine') selected @endif>Tamilnadu Cusine</option>
                                        <option value="Continental Cusine" @if(isset($member->diet) && !empty($member->diet) && $member->diet == 'Continental Cusine') selected @endif>Continental Cusine</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-xl-6 col-sm-6">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="country" name="country" placeholder="" value="{{isset($member->country) && !empty($member->country) ? $member->country : '' }}">
                                </div>
                            </div>
                        </div>
                    </div>
