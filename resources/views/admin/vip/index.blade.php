...
@section('title_bar', 'VIP Members')
@section('breadcrumb', 'VIP Members')
@section('maincontent')
                ...
                <div class="col-lg-6">
                    <div class="row mt-4">
                        ...
                        <div class="col-lg-1" style="padding:0px 15px 0px 15px;">
                            <a class="btn btn-dark" href="{{ url('admin/vip-members') }}"><i class="fa fa-refresh"></i></a>
                        </div>
                ...
                <div class="col-xl-12">
                    <div class="card" id="accordion-one">
                        <div class="card-header flex-wrap px-3">
                            <div>
                                <h4 class="card-title">VIP Member</h4>
                            </div>
                        </div>
                        <!--tab-content-->
                            ...
                            <div class="table-responsive">
                                <table id="example" class="display table">
                                    ...
                                    <tbody>
                                        ...
                                        @foreach($member as $m)
                                        <tr>
                                            <td><a href="{{ url('admin/vip-members/detail') }}{{ '/'.$m->id }}"><span class="text-primary font-w600">{{ isset($m->membership_no) && !empty($m->membership_no) ? 'RHIN'.'0000'.$m->id : 'RHIN'.'0000'.$m->id }}</span></a></td>
                                            <td>{{ $m->name }}</td>
                                            ...
                                            <td>
                                                <div class="d-flex">
                                                    <a href="{{ url('admin/vip-members/detail') }}{{ '/'.$m->id }}" data-id="{{ $m->id }}" class="btn btn-primary shadow btn-xs sharp me-1"> <i class="fa-regular fa-eye"></i></a>
                                                    <a href="{{ url('admin/vip-members/edit') }}{{ '/'.$m->id }}" class="btn btn-secondary shadow btn-xs sharp me-1"><i class="fa fa-edit"></i></a>
...
<script>
    $(document).on("click",".delete",function() {
        ...
        $('#delrecord #id').val(id);
        $('#delrecord #delete-product').attr('action', "{{ url('admin/delete-vip-members') }}");
    });
    $(document).on("click","#search-btn",function() {
        $('#search').attr("action", "{{ url('admin/vip-members') }}");
        $('#search').submit();
    });
    $(document).on("click","#export-btn",function() {
        $('#search').attr("action", "{{ url('admin/vip-members/export') }}");
        $('#search').submit();
    });
