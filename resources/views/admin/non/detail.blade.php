...
@if(isset($payment) && !empty($payment))
        ...
        <div class="row">
            ...
            @foreach($payment as $p)
             @php
            $addition_package = \App\Models\AdditionalPackage::where('member_id', $p->member_id)->get();
            @endphp
            <div class="col-xl-6">
                <div class="card">
                    <div class="card-header">
                    ...
                    <div class="card-body">
                           ...
                           &#8226;{{ isset($plan->category) && !empty($plan->category) ? $plan->category : '' }}<br/> Plan Date : {{ isset($plan->plan_date) && !empty($plan->plan_date) ? date("F d, Y", strtotime($plan->plan_date)) : '' }} </li>
                                @foreach($addition_package as $package)
                                     ...
                                     @elseif(isset($package->gala_dinner_price) && !empty($package->gala_dinner_price) && $package->gala_dinner_price !=0.00)
                                      <span><br>&#8226; Gala Dinner</span>
                                       @endif
                                      @if(!empty($package->room_tarrif_price) && isset($package->room_tarrif_price) && !empty($package->room_tarrif_price) && $package->room_tarrif_price != 0.00)
