                        ...
                        <form action="javascript:void(0)" name="registration-form" id="registration-form" class="fs-form" method="POST" enctype="multipart/form-data">
                            ...
                            <fieldset>
                                ...
                                <div class="row">
                                    ...
                                    <div class="inter_cat" @if(isset($_GET['category']) && !empty($_GET['category']) && $_GET['category'] == 'international') style="display:block" @else style="display:none" @endif>
                                        <div class="row">
                                        <div class="col-lg-4">
                                            <div class="fs-field">
                                                <label class="fs-label" for="first_name">First Name <span style="font-size:12px;color:#AD0E60;">(Name as Per Passport) <span class="h6 text-danger">*</span></span></label>
                                    ...
                                    <div class="normal_pincode" @if(isset($_GET['category']) && !empty($_GET['category']) && $_GET['category'] == 'international') style="display:none" @else style="display:block" @endif>
                                        <div class="row">
                                        <div class="col-lg-4">
                                            <div class="fs-field">
                                                <label class="fs-label" for="name">Name <span class="h6 text-danger">*</span></label>
                                    ...
                                    <div class="col-lg-4">
                                        <div class="fs-field">
                                            <label class="fs-label" for="email">Email <span class="h6 text-danger">*</span></label>
                                    ...
                                    <div class="col-lg-4">
                                        <div class="fs-field">
                                            <label class="fs-label" for="mobile">Mobile <span class="h6 text-danger">*</span></label>
                                            <input class="fs-input" id="mobile" type="tel" name="mobile" required onkeypress="return isNumberKey(event,this)"/>
    								...
    								<div class="col-lg-12">
    									...
    									<div class="fs-field">
    										<label class="fs-label" for="{{ 'training_'.$keyt }}">
    											<input class="fs-checkbox training_data" name="training[]" id="{{ 'training_'.$keyt }}" type="checkbox" value="{{ $t->text }}" data-id="{{ $t->id }}" data-price="{{ $t->price }}" data-usd_price="{{ $t->usd_price }}" data-total_user="{{ $t->user }}" data-join_user="{{ $t->join_user }}" /> <span style="position:relative;top:-6px;left:10px;">{{ $t->text }}</span>
    										</label>
    									</div>
    									@endif
    									@endforeach
                                ...
                                <div class="row ">
                                    ...
                                    <input type="hidden" name="become_airs_price" id="become_airs_price" value="4000">
...
$(document).ready(function(){
    ...
    //         var countryData = iti.getSelectedCountryData();
    //         $("#country_code").val("+" + countryData.dialCode); // e.g. +354 for Iceland
    //     }
    //     updateCountryCode();
...
$("#mobile_hidden").val(fullNumber);
        ...
        if ($("form#registration-form").valid()) {
            ...
            $("form#registration-form").attr('action', "{{ route('registration.form.store') }}");
            $("form#registration-form").submit();
        //   $.ajax({
        //     url: "{{ route('registration.form.store') }}",
        //     method: "POST",
        //     data: form.serialize(),
        ...
        //             // Instead of resubmitting form, redirect or open Razorpay
      ...
      //  console.log(room_tarrif_date.length,room_tarrif);
        ...
        if(category == "international"){
            ...
            $('.training_data:checkbox:checked').each(function() {
                const price = parseFloat($(this).data('usd_price')) || 0;
    ...
    $(document).on("click","input[name='room_tarrif']",function() {
        if($("input[name='room_tarrif']:checked").is(':checked')){
            ...
            if(room_tarrif_data == 'single_bed'){
                ...
                // Count how many are checked (3 in this case)
            let countChecked = $(".room_tarrif_date:checked").length;
            let totalPrice = countChecked * room_tarrif;
...
*/
    ...
    $('#room_tarrif_txt').val(room_tarrif_data);
    if(category == "international"){
        $('.inter_cat').css('display','block');
        $('.normal_pincode').css('display','none');

        $('#room_tarrif_amount_td').text('$ '+room_tarrif);
    } else {
        $('.inter_cat').css('display','none');
        $('.normal_pincode').css('display','block');				
        ...
        if(category !=''){
            ...
            $.get("{{ url('get-plan-detail') }}"+"/"+category, function(data, status){
                ...
                if(data.plan.category == "international" || data.plan.category == "International"){
                    ...
                    $('#dinner_price_data').text('Faculty cum Paid Dinner $ {{ isset($dinner->usd_price) && !empty($dinner->usd_price) ? $dinner->usd_price.' '.$dinner->text : 0 }}');
                    ...
                    $('#plan_amount_p').text(' ₹ '+data.plan.amount);
                    $('#dinner_price_data').text('Faculty cum Paid Dinner ₹ {{ isset($dinner->price) && !empty($dinner->price) ? $dinner->price.' '.$dinner->text : 0 }} ');
                ...
                total = (parseFloat(data.plan.amount) + gst + parseFloat(become_airs));
    ...
    */
    ...
    if(category !=''){
        ...
        $.get("{{ url('get-plan-detail') }}"+"/"+category, function(data, status){
            ...
            if(data.plan.category == "international" || data.plan.category == "International"){
                $('#plan_amount_p').text(' $ '+data.plan.amount);
                $('#dinner_price_data').text('Faculty cum Paid Dinner @if(isset($_GET["category"]) && !empty($_GET["category"]) && $_GET["category"] == "international") {{ isset($dinner->usd_price) && !empty($dinner->usd_price) ? "$ ".$dinner->usd_price.' '.$dinner->text : 0 }}  @else {{ isset($dinner->price) && !empty($dinner->price) ? "₹ ".$dinner->price.' '.$dinner->text : 0 }} @endif');
...
//             // You can redirect or show message accordingly
//         }
//     };
//     //console.log(options);
