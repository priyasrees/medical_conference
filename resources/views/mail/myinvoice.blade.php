				...
				<table class="table table-bordered  border-secondary mb-0" width="100%">
					<tbody>
						...
						@php 
						    ...
						    $overall_tax += isset($plan->amount) && !empty($plan->amount) ? (float)(($plan->amount * 18) / 100) : 0;
						@endphp
						<tr>
						...
						@endif
						@php
    $overall_total = 0;$train_price =0; // Make sure it's initialized before the foreach
@endphp
...
@endif
						...
						<tr>
							...
							@php
                                    ...
                                    foreach ($packages as $package) {
                                        if (!empty($package->training)) {
                                            $allTrainingNames = array_merge(
                                                $allTrainingNames,
                                                explode(',', $package->training)
                                            );
                                        }
                                    }
                                
                                    $allTrainingNames = array_unique($allTrainingNames); // remove duplicates
                                
                                    $trainings = \App\Models\Training::whereIn('text', $allTrainingNames)->get(['text', 'price']);
                                @endphp
                                @foreach($packages as $package)
                                    @php
                                        $trainingNames = explode(',', $package->training);
                                        $trainings = \App\Models\Training::whereIn('text', $trainingNames)->get(['text', 'price']);
                                
                                
                                        // Calculate room tariff tax (or 0)
                                        $room_tax = (!empty($package->room_tarrif_price) && $package->room_tarrif_price != 0.00)
                                            ? ($package->room_tarrif_price * 18) / 100
                                            : 0;
                                
                                        // Calculate gala dinner tax (or 0)
                                        $gala_tax = (!empty($package->gala_dinner_price) && $package->gala_dinner_price != 0.00)
                                            ? ($package->gala_dinner_price * 18) / 100
                                            : 0;
                                
                                        // Calculate total training tax (sum of 18% on each training price)
                                        $training_tax = $trainings->sum(function ($t) {
                                            return ($t->price * 18) / 100;
                                        });
                                 $overall_tax += $training_tax;

                                        // Add to overall tax
                                    @endphp
                                
                                    {{-- Room Tariff Tax --}}
                                    @if(!empty($package->room_tarrif_price) && $package->room_tarrif_price != 0.00)
                                        <br>
                                        {{ ucfirst($package->room_tarrif) ?? 'Room' }} Tax - ₹{{ number_format(($package->room_tarrif_price * 18) / 100, 2) }}
                                    @endif
                                    {{-- Gala Dinner Tax --}}
                                    @if(!empty($package->gala_dinner_price) && $package->gala_dinner_price != 0.00)
                                        <br>
                                        {{ ucfirst($package->gala_dinner) ?? 'Gala Dinner' }} Tax - ₹{{ number_format(($package->gala_dinner_price * 18) / 100, 2) }}
                                    @endif
                                
                                @endforeach
                                {{-- Show unique trainings with tax only once --}}
                                @foreach($trainings as $loopIndex => $_training)
                                    {{ $_training->text }} - ₹{{ number_format(($_training->price * 18) / 100, 2) }}
							...
							<td style="padding:10px;"align="centre">
							    @if(isset($member->category) && !empty($member->category) && $member->category == 'international') USD @else INR @endif {{ number_format($overall_tax ,2) }}
							    </td>
							<td colspan="2" style="padding:10px;"align="centre">@if(isset($member->category) && !empty($member->category) && $member->category == 'international') USD @else INR @endif {{ number_format($overall_tax ,2) }} </td>
						</tr>
						<tr>
							...
							<td colspan="2" style="padding:10px;height:100px;border-left-style: none;"><span style="text-align: centre;">@if(isset($member->category) && !empty($member->category) && $member->category == 'international') USD @else INR @endif {{ isset($overall_total) && !empty($overall_total) ? number_format($overall_total + $overall_tax, 2) : 0 }}</span></td>
						</tr>
